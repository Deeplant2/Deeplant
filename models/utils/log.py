...
def logFoldMean(train_loss, val_loss, train_metric, val_metric, eval_function, columns_name):
    ...
    # axis=3를 기준으로 나누기
    train_metric = [[[item[0][i], item[1][i], item[2][i]] for i in range(len(item[0]))] for item in train_metric]
    ...
    val_metric = [[[item[0][i], item[1][i], item[2][i]] for i in range(len(item[0]))] for item in val_metric]
